...
**Version**: v0.5.3 **shipped to PyPI** on 2026-08-28 from tag `v0.5.3` (`b58108f`) — 31 artifacts, publish job green. It carries one fix over 0.5.2: a generated crate is versioned by what the workspace asks of it, so a consumer that requires a version rather than `*` builds again. The tag run failed once on `curl: (35) OpenSSL SSL_connect: Connection reset by peer` reaching sh.rustup.rs inside the aarch64 cross container, taking the rest of the matrix with it through fail-fast; `gh run rerun --failed` on the same run was enough, and publish then ran from that run.

**Before it**: v0.5.2 shipped on 2026-08-27 from tag `v0.5.2` (`044473c`). Its first tag push failed the release gate on `cargo +nightly fmt --check`: two commits had landed unformatted, and the same failure had been red on every scheduled CI run since 2026-08-18 without anyone acting on it. Nothing had been published, so the tag was deleted on origin and re-cut on the formatting fix. The lesson is the one already recorded below and skipped that day — dry-run `wheels.yml` on main first; 0.5.3 was dry-run and passed on the first tag push.

**Previously**: v0.5.1 shipped on 2026-08-16 from tag `v0.5.1` (`8ea6122`) — 31 artifacts (30 wheels for Linux/macOS/Windows × Python 3.8–3.13, plus an sdist), with a GitHub release cut from the changelog section. It carries one user-visible change over 0.5.0: `colcon build` names the situation when `colcon-ros-cargo` is installed alongside this extension. v0.5.0 shipped the same day from `e017643`; trusted publishing is confirmed configured.

The release pipeline is worth dry-running before every tag: `gh workflow run wheels.yml --ref main` runs the identical jobs and skips publish on its `refs/tags/` guard, so a gate failure costs a dispatch rather than a tag. 0.5.1 was verified that way and passed on the first tag push.

The first two tag pushes failed at the release gate, and the reason is worth keeping: `test_build_and_test_package` asserted test artifacts under `<build_base>/debug/deps`, but the release runner's cargo is newer than the local one and uses the split build-dir layout — intermediates land in `<build_base>/debug/build/<pkg>/<hash>/out/` and no `deps` directory exists at all. The test now asserts that something was compiled, not where cargo filed it. It reached a tag unnoticed because `ci.yaml`'s Python job installed no Rust toolchain, so the test skipped there and ran only in the release path; that job now installs stable. Two guesses preceded the fix (`$CARGO_BUILD_TARGET` nesting under a triple, then the sccache wrapper) and both were wrong. What settled it: make the assertion print the directory listing, then dispatch `wheels.yml` on main — the same job, no tag needed, publish skipped by its `refs/tags/` guard.
**Phases**: 0–3 and 5–10 complete; Phase 4 open on multi-distro support (4.2). Release preparation (4.3) is partly done — v0.5.0 through v0.5.3 are on PyPI, the first two with GitHub releases; the crates.io publishes of the Rust crates and the community announcement remain
**Tests**: 263 Rust, 259 Python, plus 107 assertions across the testing workspaces (interfaces 17, layouts 47, scenarios 43). Zero warnings, clippy `-D warnings` clean
**Latest**: v0.5.3 on PyPI, carrying the fix below. The nightly heavy tier turned out to have been failing since 2026-08-18 for a second reason: pinning every generated crate to `0.0.0` broke consumers that require a version rather than `*`. A `[patch.crates-io]` entry redirects where a crate comes from, but cargo still checks it against the requirement — `candidate versions found which didn't match: 0.0.0` — which the commit that introduced the fixed version had explicitly assumed away. The version is now decided per package from what the workspace's manifests ask for (`_detect_pinned_packages`), so the reproducible-lock case and the pinned case both work. `layouts` covers both stampings in one build, `nested_node` being the package that pins.

Before that: issues #3–#12 closed, v0.5.0 and v0.5.1 on PyPI. Newest: `colcon build` warns when colcon-ros-cargo is installed alongside this extension — it wins package identification at priority 160 against colcon-ros's 150, so every Rust package goes to `cargo ament-build`, no bindings are generated, and the build still reports success. Reproduced in a container; the only other symptom is an `argparse.ArgumentError` about `--cargo-args`, which both extensions register. Since #10: the serde feature now reaches dependency crates and arrays longer than 32 (found by building iceoryx2's ROS demo); an interface package resolved from a `path` or `git` source is named rather than left to cargo (#11); and CI is green for the first time — the base-tier workspaces job had never run, because `ros:humble-ros-base` carries no pip, and the heavy tier never installed the upstream tier's dependencies. #12 closed as documented: an isolated virtualenv, not the package, is what strips colcon and breaks ROS's generators
