...
**Version**: v0.5.2 **shipped to PyPI** on 2026-08-27 from tag `v0.5.2` (`044473c`) — 31 artifacts, publish job green. The first push of that tag failed the release gate on `cargo +nightly fmt --check`: two commits had landed unformatted, and the same failure had been red on every scheduled CI run since 2026-08-18 without anyone acting on it. Nothing had been published, so the tag was deleted on origin and re-cut on the formatting fix.
...
**Phases**: 0–3 and 5–10 complete; Phase 4 open on multi-distro support (4.2). Release preparation (4.3) is partly done — v0.5.0, v0.5.1 and v0.5.2 are on PyPI, the first two with GitHub releases; the crates.io publishes of the Rust crates and the community announcement remain
