...
- **The same constant name in two action sections is covered, with a caveat.** `Execute.action` declares `NONE` in both result and feedback, as newer nav2 actions do. The values have to differ: rosidl's parser binds constants to sections by structural equality, so two identical declarations are both filed under the first section and the C generator emits a duplicate enumerator. (Investigated under issue #6; the mechanism is in `docs/troubleshooting.md`.)
