...
**Version**: v0.5.0 **shipped to PyPI** on 2026-08-16 — 31 artifacts (30 wheels for Linux/macOS/Windows × Python 3.8–3.13, plus an sdist), published from tag `v0.5.0` (`e017643`) by `wheels.yml` through trusted publishing, which is confirmed configured. `pip install colcon-cargo-ros2` now gets 0.5.0.
...
**Phases**: 0–3 and 5–10 complete; Phase 4 open on multi-distro support (4.2). Release preparation (4.3) is partly done — v0.5.0 is on PyPI; the crates.io publishes of the Rust crates and the community announcement remain
